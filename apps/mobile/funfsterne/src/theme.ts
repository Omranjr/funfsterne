...
/**
 * Fünf Sterne — brand theme.
 *
 * Two palettes (the app has always shipped a light and a dark theme with a
 * user-facing toggle), each with an explicit `tokens` block.
 *
 * Why the `tokens` blocks are spelled out rather than derived: these values
 * were hand-tuned against this palette and measured on device — the WCAG
 * contrast ratios in the comments below are real measurements, and the
 * texture alpha was raised from the design reference specifically because
 * the reference value quantises away on a phone OLED. `createThemes` would
 * derive *close* values, but "close" would be a silent visual regression on
 * a shipped app. Passing them literally makes the migration provably
 * appearance-preserving.
 *
 * A brand-new customer does the opposite: fills in the fields above the
 * `tokens` block and omits it entirely.
 */
