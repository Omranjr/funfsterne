...
import { getConfig } from "../brand/registry";
import type { Platform as PlatformType } from "@funfsterne/shared-types";

/**
 * The API origin for the tenant this build belongs to.
 *
 * Read from the registered brand on every call rather than captured in a
 * module constant: `configureBrand` runs at the app entry point, which is
 * evaluated after this module, so a constant would freeze the placeholder.
 */
 ...
 *
 * `x-tenant-id` is the whole multi-tenancy contract on the client side: the
 * API resolves it to a tenant row and scopes every query to it. A request
 * without it is rejected with 400 rather than silently answered from some
 * default tenant, so forgetting it fails loudly in development instead of
 * leaking across customers in production.
 */
function tenantHeaders(): Record<string, string> {
  return {
    "Content-Type": "application/json",
    "x-tenant-id": getConfig().tenantId,
  };
}
