...
import { getBrandTranslations, storageKey } from "../brand/registry";
...
/**
 * Base translations, shared by every customer.
 *
 * A brand's own copy -- its tagline, its hero eyebrow, its support number --
 * is layered on top per language by `applyBrandTranslations` below, using
 * i18next's own resource bundles rather than a hand-rolled deep merge. That
 * keeps the shared bundle the single source of truth for every string a
 * customer has NOT overridden, so adding a screen does not mean editing
 * every customer's locale files.
 */
...
function applyBrandTranslations(): void {
  const overrides = getBrandTranslations();
  for (const [lang, bundle] of Object.entries(overrides)) {
    if (!bundle) continue;
    // deep: merge into the nested shared bundle; overwrite: brand wins.
...
export async function initI18n(): Promise<LanguageCode> {
  applyBrandTranslations();
  const stored = await AsyncStorage.getItem(languageKey());
