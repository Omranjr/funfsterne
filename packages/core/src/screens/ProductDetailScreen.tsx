...
export default function ProductDetailsScreen() {
  ...
  // Deep links (<scheme>://products/<id>) can open this screen with no
  ...
  const handleShare = useCallback(async () => {
    if (!product) return;
    try {
      await Linking.openURL(config.instagramUrl ?? "");
    } catch (error) {
  ...
  const handleContact = useCallback(async () => {
    const url = Platform.select({
      ios: `sms:${config.supportPhone ?? ""}`,
      android: `sms:${config.supportPhone ?? ""}`,
      default: `tel:${config.supportPhone ?? ""}`,
    });
    try {
      ...
      Alert.alert(
        ...
        t("productDetail.linkFailedContact", {
          phone: config.supportPhoneDisplay ?? config.supportPhone ?? "",
        }),
      );
    }
  }, [t]);
