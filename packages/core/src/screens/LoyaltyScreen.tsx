...
import { typography, borderRadius, screenTopPadding } from "../theme/tokens";
import { useAuth } from "../stores/AuthContext";
import { Card, Button, Badge, EmptyState, Ground } from "../components";
import { useLoyaltyMe } from "../hooks/usePublicData";
import { redeemLoyaltyPoints, PublicApiError } from "../api/client";
import { queryClient } from "../stores/query-client";

// The QR payload staff scan in the admin dashboard. Namespaced by tenant
// so one shop's scanner physically cannot award points against another
// shop's customer id -- the dashboard checks the prefix before parsing.
const qrPrefix = (tenantId: string) => `${tenantId}:loyalty:`;
...
export default function LoyaltyScreen() {
  ...
  const qrValue = user ? `${qrPrefix(config.tenantId)}${user.id}` : "";
