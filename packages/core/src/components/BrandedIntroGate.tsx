...
import { getIntroVariant } from "../brand/registry";
...
const FADE_OUT_MS = 600; // both intros use the same fade-out duration

/**
 * How long each opening needs to play out before it may be dismissed.
 *
 * Per variant, because the two compositions are not the same length: the
 * classic one has to get through five staggered stars and a scissor snip,
 * the editorial one settles sooner. Using the classic timing for both would
 * hold the editorial intro on a finished, motionless screen for 300ms.
 */
const MIN_VISIBLE_MS: Record<ReturnType<typeof getIntroVariant>, number> = {
  classic: 2900, // photo + wordmark + stars + scissors fully played
  editorial: 2600, // monogram + wordmark + divider + tagline settled
};
...
export function BrandedIntroGate({ children }: BrandedIntroGateProps) {
  ...
  // Read once: the brand cannot change while the app is running, and this
  // must not re-evaluate mid-sequence.
  const variantRef = useRef(getIntroVariant());
  ...
  useEffect(() => {
    ...
    const remainingToMin = Math.max(0, MIN_VISIBLE_MS[variant] - elapsed);
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      {children}
      {variant === "editorial" ? (
        <EditorialIntro dismissRef={dismissRef} />
      ) : (
