...
import { getThemes, storageKey } from "../brand/registry";
import { logSwallowed } from "../utils/log";

// Namespaced by tenant so two white-label builds can never read each
// other's stored preference. Resolved lazily rather than at module scope:
// Metro evaluates this module before the app entry point runs
// `configureBrand`, so a module-scope call would bake in the placeholder
// tenant id.
const themeStorageKey = () => storageKey("theme-mode");
...
export function ThemeProvider({ children }: { children: ReactNode }) {
  ...
  const setMode = useCallback((next: ThemeMode) => {
    setModeState(next);
    AsyncStorage.setItem(themeStorageKey(), next).catch((error) => {
