...
import { hasBeenPrompted } from "../utils/notification-permission";
...
import { configureBrand, type BrandDefinition } from "../brand/registry";
 ...
 * the `scheme` each customer declares in its own app.json. Notification
...
function AppNavigator() {
  ...
  // A feature a customer has switched off is hidden with `href: null`
  // rather than removed: expo-router still needs a <Tabs.Screen> for every
  // route file that exists, and `href: null` is how it is told to keep the
  // route reachable by deep link while leaving it out of the tab bar.
        ...
        <Tabs.Screen
          ...
          options={
            features.productCatalogue
              ? { title: t("tabs.shop"), tabBarIcon: tabIcon(ShoppingBag) }
              : hidden
          }
        />
        <Tabs.Screen
          name="discount-codes"
          options={
            features.discountCodes
              ? { title: t("tabs.offers"), tabBarIcon: tabIcon(TicketPercent) }
              : hidden
          }
        />
        <Tabs.Screen
          ...
          options={
            features.loyaltyPoints
              ? { title: t("tabs.rewards"), tabBarIcon: tabIcon(Star) }
              : hidden
          }
        />
        <Tabs.Screen
...
function RootLayout() {
  ...
  // Built once, from the registered brand: the persister's storage key is
  // namespaced by tenant, so it cannot be a module-level constant.
...
/**
 * Builds the root component for one white-label app.
 *
 * This is the whole seam between `packages/core` and a customer: the app's
 * `app/_layout.tsx` is one call to this, and everything else it owns is
 * data (`src/theme.ts`, `src/config.ts`, `assets/`).
 *
 * `configureBrand` is called here, synchronously, at module-evaluation time
 * of the app entry point — before anything renders and before any screen
 * module runs a `getConfig()`. Every brand read inside core is lazy for
 * exactly this reason: ES module imports are hoisted, so core's modules are
 * evaluated before this line runs, and a module-scope read would have baked
 * in the placeholder tenant.
 */
export function createAppRoot(brand: BrandDefinition) {
